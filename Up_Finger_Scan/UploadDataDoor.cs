...
namespace ATTN {
    public partial class UploadDataDoor : Form {
        public UploadDataDoor() {
            InitializeComponent();
        ...
        private void UploadUNIS() {
            try {
                using (OleDbConnection con = new OleDbConnection(cboLocal.SelectedValue.ToString())) {
                    ...
                    //string connect = cboSever.SelectedValue.ToString();
                    using (OracleConnection connection = new OracleConnection(HUBIC)) {

                        connection.Open();

                        foreach (DataRow row in dt.Rows) {
                            using (OracleCommand command = new OracleCommand("PW_TL_SCAN_DOOR_UPLOAD_S2", connection)) {
                                command.CommandType = CommandType.StoredProcedure;

                                command.Parameters.Add("ARG_SERVICE_ID", OracleDbType.Varchar2).Value = "VJ";
                                command.Parameters.Add("ARG_LOAD_DATE", OracleDbType.Varchar2).Value = row["C_DATE"].ToString();
                                command.Parameters.Add("ARG_FILE_NAME", OracleDbType.Varchar2).Value = cboLocal.Text;
                                command.Parameters.Add("ARG_MACHINE_ID", OracleDbType.Varchar2).Value = row["L_TID"].ToString();
                                command.Parameters.Add("ARG_EMP_NO", OracleDbType.Varchar2).Value = row["L_UID"].ToString();
                                command.Parameters.Add("ARG_EMPID", OracleDbType.Varchar2).Value = row["C_UNIQUE"].ToString();
                                command.Parameters.Add("ARG_IO_DIV", OracleDbType.Varchar2).Value = row["L_Mode"].ToString();
                                command.Parameters.Add("ARG_IO_TIME", OracleDbType.Varchar2).Value = row["C_DATE"].ToString() + row["C_TIME"].ToString();
                                command.Parameters.Add("ARG_FIX_TF", OracleDbType.Varchar2).Value = row["L_MatchingType"].ToString();
                                command.Parameters.Add("ARG_CHECK_FIL", OracleDbType.Varchar2).Value = "ACCESS";
                                ...
                                command.Parameters.Add("ARG_ROW_COUNT", OracleDbType.Decimal).Direction = ParameterDirection.Output;

                                int runvalue = command.ExecuteNonQuery();

                                if (command.Parameters["ARG_ROW_COUNT"].Value.ToString() != "0") {
                                    lblStatus.Text = "Upload: " + (++iRowUpload).ToString() + "/" + iRow.ToString();
                                    /*****************
                                     * Upload Status In Local
                                     *
                                     *****************/
                                    strSql = "  UPDATE TENTER  " +
                                             "     SET APPLY_YN = 'Y' " +
                                             "   WHERE APPLY_YN = 'N' " +
                                             "     AND C_DATE   = '" + row["C_DATE"].ToString() + "' " +
                                             "     AND C_TIME   = '" + row["C_TIME"].ToString() + "' " +
                                             "     AND L_TID    = " + row["L_TID"].ToString() + " " +
                                             "     AND L_UID    = " + row["L_UID"].ToString() + " " +
                                             "     AND C_UNIQUE = '" + row["C_UNIQUE"].ToString() + "' " +
                                             "     AND L_MODE   = " + row["L_Mode"].ToString() + " ";

                                    //if (_db.execSql(strSql, con, "Update Status ERROR: " + row["C_UNIQUE"].ToString()) == 0)
                                    //    WriteLog("No Update Status: " + row["C_UNIQUE"].ToString());
            ...
            try {
                using (OleDbConnection con = new OleDbConnection(cboLocal.SelectedValue.ToString())) {
                    ...
                    using (OracleConnection connection = new OracleConnection(HUBIC)) {
                        ...
                        foreach (DataRow row in dt.Rows) {
                            using (OracleCommand command = new OracleCommand("PW_TL_SCAN_DOOR_UPLOAD_S", connection)) {
                                command.CommandType = CommandType.StoredProcedure;

                                command.Parameters.Add("ARG_SERVICE_ID", OracleDbType.Varchar2).Value = "VJ";
                                command.Parameters.Add("ARG_LOAD_DATE", OracleDbType.Varchar2).Value = row["E_Date"].ToString();
                                command.Parameters.Add("ARG_FILE_NAME", OracleDbType.Varchar2).Value = cboLocal.Text;
                                command.Parameters.Add("ARG_MACHINE_ID", OracleDbType.Varchar2).Value = row["g_ID"].ToString();
                                command.Parameters.Add("ARG_EMP_NO", OracleDbType.Varchar2).Value = row["E_id"].ToString();
                                command.Parameters.Add("ARG_EMPID", OracleDbType.Varchar2).Value = row["E_idno"].ToString();
                                command.Parameters.Add("ARG_IO_DIV", OracleDbType.Varchar2).Value = row["e_Mode"].ToString();
                                command.Parameters.Add("ARG_IO_TIME", OracleDbType.Varchar2).Value = row["E_Date"].ToString() + row["E_Time"].ToString();
                                command.Parameters.Add("ARG_FIX_TF", OracleDbType.Varchar2).Value = row["E_Type"].ToString();
                                command.Parameters.Add("ARG_CHECK_FIL", OracleDbType.Varchar2).Value = "ACCESS";
                                command.Parameters.Add("ARG_IP", OracleDbType.Varchar2).Value = strIP;
                                command.Parameters.Add("ARG_USER", OracleDbType.Varchar2).Value = strUser;
                                // Output parameter
                                var errorParam = new OracleParameter("V_P_ERROR_STR", OracleDbType.Varchar2, 4000);
                    ...
                    using (OracleConnection connection = new OracleConnection(cboSever.SelectedValue.ToString())) {
                        ...
                        foreach (DataRow row in dt.Rows) {
                            using (OracleCommand command = new OracleCommand("PW_TL_SCAN_MC_UPLOAD_S", connection)) {
                                ...
                                command.Parameters.Add("ARG_SERVICE_ID", OracleDbType.Varchar2).Value = "VJ";
                                command.Parameters.Add("ARG_LOAD_DATE", OracleDbType.Varchar2).Value = row["CheckTime"].ToString();
                                command.Parameters.Add("ARG_FILE_NAME", OracleDbType.Varchar2).Value = cboLocal.Text;
                                command.Parameters.Add("ARG_MACHINE_ID", OracleDbType.Varchar2).Value = row["AliasName"].ToString();
                                command.Parameters.Add("ARG_EMP_NO", OracleDbType.Varchar2).Value = row["UserID"].ToString();
                                command.Parameters.Add("ARG_EMPID", OracleDbType.Varchar2).Value = "";
                                command.Parameters.Add("ARG_IO_DIV", OracleDbType.Varchar2).Value = row["InOutMode"].ToString();
                                command.Parameters.Add("ARG_IO_TIME", OracleDbType.Varchar2).Value = row["CheckTime"].ToString();
                                command.Parameters.Add("ARG_FIX_TF", OracleDbType.Varchar2).Value = "";
                                command.Parameters.Add("ARG_CHECK_FIL", OracleDbType.Varchar2).Value = "SQL";
                                command.Parameters.Add("ARG_IP", OracleDbType.Varchar2).Value = strIP;
                                command.Parameters.Add("ARG_USER", OracleDbType.Varchar2).Value = strUser;
        ...
        private void RunUpload() {
            ...
            if (cboLocal.Text == "SQL") { ts = new ThreadStart(UploadSQL); } else if (cboLocal.Text == "ACCESS") { ts = new ThreadStart(UploadACCESS); } else
